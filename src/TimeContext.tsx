...
export function TimeContext() {
  ...
  const forceNodes = [...Array(17).fill(1).map(index => ({ id: index }))]
  ...
  useEffect(() => {
    ...
    const simulation = d3.forceSimulation(forceData) //.force(name, method)
      // .force('center',d3.forceCenter(width/2,height/2))
    	.force('collide', d3.forceCollide().radius(10))  //collide is to prevent overlap
    	.force('x', d3.forceX().x((d,i)=> i==0?200:300)) // usually I do this in the focusX / focusY properties in the dataset
